...
void bmp280_get_data(I2C_Handle *i2c, double *pres, double *temp) {
    ...
    if (I2C_transfer(*i2c, &i2cTransaction)) {
        ...
        uint32_t pressure = 0;
        pressure = (pressure | rxBuffer[0]) << 8;
        pressure = (pressure | rxBuffer[1]) << 4;
        pressure = pressure | (rxBuffer[2] >> 4);
        
        uint32_t temperature = 0;
        temperature = (temperature | rxBuffer[3]) << 8;
        temperature = (temperature | rxBuffer[4]) << 4;
        temperature = temperature | (rxBuffer[5] >> 4);
